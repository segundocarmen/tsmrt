...
export default {
    ...
    data(){
        return {
            periodName: this.period ? this.period.periodName : '' ,
            periodYear: this.period ? this.period.periodYear : 'Año',
            periodDate: '',
            endDate: null,
            yearSelect: 'Año',
            periodDate2: ''
    ...
    watch: {
        ...
        period: {
            inmediate: true,
            handler(val) {
                this.periodYear = val.startString ? val.startString.split('-')[0] : 'Año'
                this.$emit('changeYear', { index: this.periodPosition, year: this.periodYear })
            }
        }
    },
    methods: {
        ...
        changeDataPeriod() {
            ...
            this.$emit('changeDataPeriod', {
                'periodName': this.periodName.name ? this.periodName.name : this.periodName,
                'isIrregular': this.periodName.isIrregular ? this.periodName.isIrregular : false,
                ...
                'inputsComplete': this.inputIsRequired,
                'typeOperation': this.period.idPeriod ? crudStatus.UPDATED : crudStatus.CREATED,
                'isAssociated': this.period.isAssociated
            })
        },
        deletePeriod() {
        ...
        changeName(val){
            this.$emit('changeName', { periodName: val.periodName, periodYear: val.periodYear, isIrregular: val.isIrregular,isAssociated: val.isAssociated, id: this.period.idPeriod, position: this.periodPosition, inputsComplete: this.inputIsRequired})
        },
        setEndDatesList() {
